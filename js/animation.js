...
// gsap.from('.image-hero', 1, {opacity: 0, x: 100});
...
tl.from('#window3', {duration: time*1.5, opacity: 0.5, x: -50, ease: "back.out(4)"},'window');
tl.from('#window1', {duration: time*1.5, opacity: 0.5, x: 50, ease: "back.out(4)"},'window');
...
tl.from('#code-bottom', {duration: time, scaleX:0, ease: "expo.out"}, "-=0.3");
...
tl.from('#code--window4', {duration: time*1.2, scaleX:0, ease: "expo.out"});
tl.from('#code', {duration: time*1.2, scaleX:0, ease: "expo.out"}, "-=0.4");
...
tl.add('icons')
tl.from('#icon1', {duration: time*2, opacity: 0, y: -50, ease: "elastic.out(1, 0.3)"}, 'icons1');
tl.from('#icon2', {duration: time*3, opacity: 0, y: -80, ease: "elastic.out(1, 0.3)"},'icons1');
tl.from('#icon3', {duration: time*4, opacity: 0, y: 50, ease: "elastic.out(1, 0.3)"}, 'icons1');
tl.from('#dots', {duration: time, opacity: 0, y: 80},'icons1');

tl.to('#icon1, #icon2, #icon3', {rotation:"360", transformOrigin:"center",duration: 3, ease: 'none', repeat:-1}, "-=1")
...
animateElements.forEach(animateElement  => {
    ...
    // else if (animateElement.getAttribute('data-gsap') == 'bottom'){
