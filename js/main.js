...
menuAnchors.forEach(menuAnchor => {
    menuAnchor.addEventListener('click', function(e){
        ...
        const scrollToSection = document.querySelector(`[data-content="${menuAnchor.dataset.anchor}"]`)       
    ...
    filter.addEventListener('click', () => {
        ...
        if (!(filter.classList.contains('technologies__icon--filters-active'))) {
            ...
            projectsItems.forEach(item =>{
                ...
                if (!(projectsAttr.includes(filterName))) item.classList.add('projects-listing__item--hidden')
            })
            ...
            projectsItems.forEach(item => {
                const projectsAttr = item.getAttribute('data-usedTechnologies');
                if (!(projectsAttr.includes(filterName))) item.classList.remove('projects-listing__item--hidden');
            })
            filter.classList.remove('technologies__icon--filters-active');
            numberOfActiveFIlters--;

            if(!numberOfActiveFIlters && clearBtn.classList.contains('filters__clearBtn--active')) clearBtn.classList.remove('filters__clearBtn--active');
